...
import { getPopularItems, getBudgetandSales } from '../../SharedComponents/ReusableFunctions';
import { getUserRequest } from '../../../services/axios/UserRequests';
...
const NavigationBar = (props) => {
    ...
    if (!loaded) {
        ...
        getUserPromise.then(userResponse => {
            ...
            const popularItems = getPopularItems(userResponse.data.items, userResponse.data.popularItems);
            const [budgetItems, newArrivals, saleItems] = getBudgetandSales(userResponse.data.items);
            ...
            props.setBudgetItems(budgetItems);
            props.setNewArrivals(newArrivals);
            props.setPopularItems(popularItems);
            props.setSaleItems(saleItems);
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        setBudgetItems: (items) => {
            dispatch({type: "shop/setBudgetItems", payload : items})
        },

        setNewArrivals: (payload) => {
            dispatch({type: "shop/setNewArrivals", payload})
        },

        setPopularItems: (items) => {
            dispatch({type : "shop/setPopularItems", payload : items})
        },

        setSaleItems: (items) => {
            dispatch({type: "shop/setSaleItems", payload : items})
        },
