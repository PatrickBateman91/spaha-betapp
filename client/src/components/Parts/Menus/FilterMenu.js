                ...
                {item.values.map((filter) => {
                    return (
                        <div className={item.filterName === "size" ? "sizes-menu-box fx-basic fx-justify-center" : "filter-menu-box fx-basic fx-justify-between"} key={filter}>
                            ...
                            <div>
                                <label id={`label-${item.filterName}-${filter.toLowerCase()}`} htmlFor={`${item.filterName}-${filter.toLowerCase()}`}>{filter}</label>
                            </div>

                            <div className="filter-input">
                                <input name={`${item.filterName}`} id={`${item.filterName}-${filter.toLowerCase()}`} type={item.type} value={filter} onClick={e => props.handleFilter(e, item.filterName, filter.toLowerCase())} />
                            </div>
                        </div>
                    )
                })}
