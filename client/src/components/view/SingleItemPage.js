...
import EditItemButton from '../dumbComponents/Buttons/EditItemButton';
import CardRating from '../dumbComponents/CardRating';
import LoginModal from '../Auth/LoginModal';
import BuyButton from '../dumbComponents/Buttons/BuyButton';
import Sizes from '../dumbComponents/Sizes';
import ShoppingItem from './ShoppingItem';
import {addToCartVANJSKA, checkCorrectMailFormat, passwordCheck, selectRating} from '../dumbComponents/ReusableFunctions';
...
handleAmount = (e) => {
    ...
    if(e.target.value > this.state.item.availability[this.state.selectedSize]){
        this.setState({selectedAmount : parseInt(e.target.value),
            error : true,
            errorMessage:`We only have ${this.state.item.availability[this.state.selectedSize]} ${this.state.item.availability[this.state.selectedSize] === 1 ? "pair" : "pairs"} of selected sizes available!`,
        })
    }
